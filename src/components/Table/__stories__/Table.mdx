...
`field` - поле, к которому применяется фильтрацию

`filterer` - функция фильтрации

`filterComponent` - компонент, в котором задается значение фильтра

`filterComponentProps` - пропсы для `filterComponent`
...
```tsx
{
 name:{
    id: 'executor',
    filterer: (cellValue, filterValue) => value === filterValue.name,
    field: 'name',
    filterComponent: TableCheckboxGroupFilter,
    filterComponentProps: {'title': 'Выбрать имена', items: [{name: 'Антон', {value: 'anton'}, {name: 'Марина', value: 'marina'}}]}
  },
  ...
  age: {
    ...
    filterComponentProps: {'title': 'Допустимый возраст'}
    initialValue: {min: 18, max: undefined}
...
type Filter<T extends TableRow> = {
  id: string;
  field: RowField<T>;
  filterer: (value: FieldValue, filterValue: FilterValue) => boolean;
...
type Filters<T extends TableRow> = {
  [key in RowField<T>] :Filter<T>
}

type SelectedFilters<T extends TableRow> = {
...
| [`filters`](#фильтры)                               | `Filters`                                           | -                                                 | Список фильтров                                      |
...
| [`onFiltersUpdated`](#колбэк-на-применение-фильтра) | `(filters: SelectedFilters) => void`                | -                                                 | Колбэк на изменение фильтров                         |
